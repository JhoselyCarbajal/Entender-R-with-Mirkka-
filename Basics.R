#x <- #La flecha es para nombrar una variable/valor
#Siempre crear un New Project, no un New File

#Crear un vector
### (1)Create a vector with numbers from 1 to 31. 
### With this vector and the command paste() *PUT SOMETHING TOGHETER* create a vector named “tree.name” 
...
Vector1 <- c(1,2,3,4,5)
Jhosely <- paste0("Jhos_", 1,2,3,4,5)

Vector2 <- c(1:5)
Diana <-paste("Dianis_", 1:5) 

#$ Sirve para dar nombre a cualquier variable. #$ sirve para ponerlo después del dataframe, saber que variables ya tienes y elegir
#Data.frame es para crear el DataFrame

d0$new.col <- new.col

#Corchetes [] es para la ubicación
#== es para comparar ambas cosas "esto es igual a esto otro"
# max es para obtener el valor más alto, Ex. el árbol más alto
...
#<- ASIGNAR VALORES A LA IZQUIERDA
